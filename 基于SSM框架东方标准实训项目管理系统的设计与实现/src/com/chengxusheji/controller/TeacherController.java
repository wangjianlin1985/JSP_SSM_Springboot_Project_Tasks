...
public class TeacherController extends BaseController {
	...
	public void list(String teacherNo,String name,String bornDate,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (teacherNo == null) teacherNo = "";
		if (name == null) name = "";
		if (bornDate == null) bornDate = "";
		if(rows != 0)teacherService.setRows(rows);
		List<Teacher> teacherList = teacherService.queryTeacher(teacherNo, name, bornDate, page);
	    /*计算总的页数和总的记录数*/
	    teacherService.queryTotalPageAndRecordNumber(teacherNo, name, bornDate);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String teacherNo,String name,String bornDate,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (teacherNo == null) teacherNo = "";
		if (name == null) name = "";
		if (bornDate == null) bornDate = "";
		List<Teacher> teacherList = teacherService.queryTeacher(teacherNo, name, bornDate, currentPage);
	    /*计算总的页数和总的记录数*/
	    teacherService.queryTotalPageAndRecordNumber(teacherNo, name, bornDate);
	    /*获取到总的页码数目*/
	    int totalPage = teacherService.getTotalPage();
	...
	public void OutToExcel(String teacherNo,String name,String bornDate, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(teacherNo == null) teacherNo = "";
        if(name == null) name = "";
        if(bornDate == null) bornDate = "";
        List<Teacher> teacherList = teacherService.queryTeacher(teacherNo,name,bornDate);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Teacher信息记录"; 
        String[] headers = { "教师编号","姓名","性别","出生日期","教师照片"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<teacherList.size();i++) {
        	Teacher teacher = teacherList.get(i); 
        	dataset.add(new String[]{teacher.getTeacherNo(),teacher.getName(),teacher.getSex(),teacher.getBornDate(),teacher.getTeacherPhoto()});
        }
        /*
