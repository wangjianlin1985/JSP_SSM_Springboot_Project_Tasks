...
public class ClassInfoController extends BaseController {
	...
	public void list(String classNo,String className,String bornDate,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (classNo == null) classNo = "";
		if (className == null) className = "";
		if (bornDate == null) bornDate = "";
		if(rows != 0)classInfoService.setRows(rows);
		List<ClassInfo> classInfoList = classInfoService.queryClassInfo(classNo, className, bornDate, page);
	    /*计算总的页数和总的记录数*/
	    classInfoService.queryTotalPageAndRecordNumber(classNo, className, bornDate);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String classNo,String className,String bornDate,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (classNo == null) classNo = "";
		if (className == null) className = "";
		if (bornDate == null) bornDate = "";
		List<ClassInfo> classInfoList = classInfoService.queryClassInfo(classNo, className, bornDate, currentPage);
	    /*计算总的页数和总的记录数*/
	    classInfoService.queryTotalPageAndRecordNumber(classNo, className, bornDate);
	    /*获取到总的页码数目*/
	    int totalPage = classInfoService.getTotalPage();
	...
	public void OutToExcel(String classNo,String className,String bornDate, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(classNo == null) classNo = "";
        if(className == null) className = "";
        if(bornDate == null) bornDate = "";
        List<ClassInfo> classInfoList = classInfoService.queryClassInfo(classNo,className,bornDate);
