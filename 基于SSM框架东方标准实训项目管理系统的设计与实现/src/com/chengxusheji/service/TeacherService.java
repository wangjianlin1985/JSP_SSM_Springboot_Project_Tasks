...
public class TeacherService {
    ...
    /*按照查询条件分页查询教师记录*/
    public ArrayList<Teacher> queryTeacher(String teacherNo,String name,String bornDate,int currentPage) throws Exception { 
    	...
    	if(!bornDate.equals("")) where = where + " and t_teacher.bornDate like '%" + bornDate + "%'";
    	int startIndex = (currentPage-1) * this.rows;
    	return teacherMapper.queryTeacher(where, startIndex, this.rows);
    }

    /*按照查询条件查询所有记录*/
    public ArrayList<Teacher> queryTeacher(String teacherNo,String name,String bornDate) throws Exception  { 
     	String where = "where 1=1";
    	if(!teacherNo.equals("")) where = where + " and t_teacher.teacherNo like '%" + teacherNo + "%'";
    	if(!name.equals("")) where = where + " and t_teacher.name like '%" + name + "%'";
    	if(!bornDate.equals("")) where = where + " and t_teacher.bornDate like '%" + bornDate + "%'";
    ...
    /*当前查询条件下计算总的页数和记录数*/
    public void queryTotalPageAndRecordNumber(String teacherNo,String name,String bornDate) throws Exception {
     	String where = "where 1=1";
    	if(!teacherNo.equals("")) where = where + " and t_teacher.teacherNo like '%" + teacherNo + "%'";
    	if(!name.equals("")) where = where + " and t_teacher.name like '%" + name + "%'";
    	if(!bornDate.equals("")) where = where + " and t_teacher.bornDate like '%" + bornDate + "%'";
	...
	/*验证用户登录*/
	public boolean checkLogin(String teacherNo, String password) throws Exception { 
		Teacher db_teacher = (Teacher) teacherMapper.getTeacher(teacherNo);
		if(db_teacher == null) { 
			this.errMessage = " 账号不存在 ";
			System.out.print(this.errMessage);
			return false;
		} else if( !db_teacher.getPassword().equals(password)) {
